...
import type { GetDataFromSaveDataType, ExportEditedSaveDataType } from '$types';
...
export class BinaryManager {
     ...
     */
    private overwriteBuffer(sourceBuffer: Buffer, offset: number, data: Buffer): Buffer {
     ...
     */
    public getDataFromSavedata(type: GetDataFromSaveDataType, savedata: string | null): number {
        if (!savedata) {
            return 0;
        }
        ...
        switch (type) {
            case 'playtime': {
                const rawSavedata = this.decompress(Buffer.from(savedata, 'hex'));
     ...
     */
    public exportEditedSavedata(type: ExportEditedSaveDataType, savedata: Buffer, srcBuffer: Buffer): string {
        const { decompress, overwriteBuffer, compress } = this;

        switch (type) {
            case 'name': {
                const rawSavedata = decompress(savedata);
                const editedSavedata = overwriteBuffer(rawSavedata, 0x58, srcBuffer);
                const base64 = compress(editedSavedata).toString('base64');
