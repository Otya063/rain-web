<script lang="ts">
    ...
    const menuTypes: CategoryType = {
        user: false,
        character: false,
    };
    const menuModeHandle = (id: number, type: keyof CategoryType) => {
        editId = id;
        menuTypes[type] = !menuTypes[type];
    };
</script>
...
<div class="console_contents">
    ...
    {:else}
        {#each _.sortBy(users_data, 'id') as user, i}
            ...
            <button class="console_contents_list_button" on:click={() => menuModeHandle(user.id, 'user')}>View User Data ▶</button>
            {#if editId === user.id && menuTypes['user']}
                    ...
                    {:else}
                        {#each characters_data as character}
                            {#if user.id === character.user_id && !character.is_new_character}
                                ...
                                <dd class="contents_desc">{character.id}{user.id}{character.user_id}</dd>
