<!-- 未使用コンポーネント 2026/06/25 -->
<!-- <script lang="ts">
    import { applyAction, enhance } from '$app/forms';
    import { allBannerData, onSubmit, closeModal, modalData, conv2DArrayToObject, msgClosed, timeOut, closeMsgDisplay, checkModalType } from '$utils/client';
</script>

{#if checkModalType('deleteBanner', $modalData)}
    <div class="modal">
                ...
                use:enhance={({ formData }) => {
                    ...
                    return async ({ result }) => {
                        ...
                        if (result.type === 'success') {
                            $allBannerData = $allBannerData.filter((bnr) => bnr.id !== id);
                        }
